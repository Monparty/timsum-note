...
function TimsumContainer({ displayPalette, setDisplayPalette, items, setDroppedItems, isShowObj = false }) {
    useEffect(() => {
        const bars = ["#bar-left", "#bar-right"].map((id) => document.querySelector(id));
    ...
    return (
        <div className="h-full relative">
            {isShowObj && (
                <>
                    ...
                    <Image
                        src={catHand}
                        id="bar-left"
                        className="absolute bottom-2 left-18 z-10"
                        alt="cat hand"
                        width={120}
                    />
                    <Image
                        src={catHand}
                        id="bar-right"
                        className="absolute bottom-2 right-18 z-10"
                        alt="cat hand"
                        width={120}
                    />
                </>
            ...
            <TimsumCat
                className="absolute -bottom-4 left-1/2 -translate-x-1/2 select-none [-webkit-user-drag:none] cursor-grab"
                width={120}
            />
            <Image src={pc} id="bar-right" className="absolute bottom-1 left-5 z-30" alt="cat hand" width={120} />
